...
| Azure control-plane identity | Kubernetes TokenReview and ServiceAccount bindings authenticate lifecycle and sandbox callers. The host-side demo uses reusable audience-scoped bearer tokens until expiry. | Integrate Microsoft Entra ID, managed identities, workload identity federation, on-behalf-of user authorization, private endpoints, mTLS, certificate rotation, and Azure RBAC. Use Pod-bound projected tokens where the caller is a workload and do not accept caller-asserted tenant context. |
| Service-to-service transport | assignmentd, OpenSandbox, and the authorization probe communicate over cluster-local plaintext HTTP or gRPC in the POC. | Require mutually authenticated and automatically rotated TLS identities for every control-plane hop. Reject plaintext production configuration and remove shared API keys where workload identity or token exchange can be used. |
| Runtime admission after control-plane compromise | The trusted template path enforces Kata, a dedicated workload ServiceAccount, and no automatic token mount. The upstream OpenSandbox server still has namespace Pod lifecycle permissions. | Separate control-plane and workload identities and enforce admission independently of OpenSandbox so a compromised lifecycle component cannot create non-Kata, privileged, host-networked, or host-mounted workloads. |
